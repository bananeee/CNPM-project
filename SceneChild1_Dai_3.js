class SceneChild1 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image('left', 'assets/trolley_left.png');
        this.load.image('right', 'assets/trolley_right.png');
    }
    ...
    create() {
        ...
        // this.label = this.add.text(0, 0, '', { font: "48px Arial Black", fill: "#c51b7d" });
    ...
    // Drop zone 
    setUpDropzone() {
        // this.zoneWeekDay = this.add.zone(0.23 * config.width, 0.78 * config.height, 0.46 * config.width, 0.46 * config.height).setName("Weekdays").setRectangleDropZone(0.46 * config.width, 0.46 * config.height);
        // this.zoneHoliay = this.add.zone(0.77 * config.width, 0.78 * config.height, 0.46 * config.width, 0.46 * config.height).setName("Weekends").setRectangleDropZone(0.46 * config.width, 0.46 * config.height);

        this.zoneWeekDay = this.add.image(0.23 * config.width, 0.78 * config.height, "left").setName("Weekdays").setInteractive();
        this.zoneHoliay = this.add.image(0.77 * config.width, 0.78 * config.height, "right").setName("Weekends").setInteractive();
        this.zoneHoliay.input.dropZone = true;
        this.zoneWeekDay.input.dropZone = true;
        this.zoneWeekDay.setDisplaySize(0.46 * config.width, 0.46 * config.height);
        this.zoneHoliay.setDisplaySize(0.46 * config.width, 0.46 * config.height);
    ...
    packageSetup() {
        for (let i = 0; i < 3; i++) {
            this.packageStacked.push(
                this.add.image(-100, 200, 'pk' + i).setName("Weekdays").setInteractive({ draggable: true }),
            ); //Start position
        }
    }
    ...
    move(speed) {
        ...
        while (i < this.packageOnTrack.length) {
            if (this.packageOnTrack[i].x > config.width + 208 / 2) { //end position  
                this.packageOnTrack[i].x = -100; //restart position
