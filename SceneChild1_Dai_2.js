class SceneChild1 extends Phaser.Scene {
    ...
    setUpDropzone() {
        ...
        this.zoneHoliay = this.add.zone(0.77 * config.width, 0.78 * config.height, 0.46 * config.width, 0.46 * config.height).setRectangleDropZone(0.46 * config.width, 0.46 * config.height);
    ...
    packageSetup() {
        for (let i = 0; i < 3; i++) {
            this.packageStacked.push(
                this.add.image(-100, 200, 'pk' + i).setInteractive({ draggable: true })); //Start position
        }
    }
