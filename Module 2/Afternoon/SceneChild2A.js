class SceneChild2A extends Phaser.Scene {
    ...
    gameSetup() {
        ...
        // Hiển thị thời gian
        this.timeTextA = ["7:30", "8:00", "10:00", "12:30", "14:00", "17:30", "19:00", "21:00"];
        for (let i = 0; i <= 7; i++) {
            this.add.text(this.coordinateImage.x[i],
                this.coordinateImage.y[i] - 0.24 * config.height,
                this.timeTextA[i]).setFontFamily('Arial').setFontSize(30).setColor('#000000').setOrigin(0.5)
        }

        this.captionScene2A_1 = this.make.text({
            x: 0.5 * config.width,
        ...
        this.captionScene2A_2 = this.make.text({
            x: 0.5 * config.width,
            y: 0.2 * config.height,
